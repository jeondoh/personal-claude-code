...
User-facing artifacts: **PRD**, **Design Doc**, **ADR**, **Diagnose** — tracked in `docs/*`, consumed by humans + future personas. Internal workflow files (tickets, reviews, inbox, handoff) live under `.claude-team/` → see `ticket-protocol`.

## Language and metadata policy

- **Body** + Mermaid labels: Korean (user-consumed).
- **Frontmatter** (YAML keys + enums), **identifiers** (file names, AC IDs, ADR numbers): English.
...
`medium` auto-escalating to `large` mid-flight → split the merged spec into separate PRD + Design at that point; do not retrofit history.

## ADR triggers — create one if **any** apply (when in doubt, create it)

1. **External dependency change** — add/replace library, framework, payment provider, external API.
2. **Storage or data-flow change** — DB engine swap, sync→async pipeline, cache/event-bus introduction.
3. **Public/shared interface change** — contract used by 3+ callers, cross-module DTO change, public API surface.
4. **Auth/permission/security-policy change** — AuthN/AuthZ flow, secret handling, compliance scope.

If created, the Design Doc must list it under **Prerequisite ADRs**.

## Document specs

### PRD — `docs/prd/<feature-slug>-prd.md`

**Author**: Spec Shaman. **Approval**: Stop step in `large` only (medium folds into one combined Stop).

**Required sections**:

1. **배경 / 문제** — what's broken/missing, who hurts, why now
2. **목표 / Non-goals** — outcome statements; non-goals explicit
3. **사용자 가치 / 핵심 시나리오** — 1–3 user journeys
4. **Acceptance Criteria** — `AC-001`, `AC-002`, … each with a measurable pass condition. Sequential IDs referenced from Design Doc and tickets.
5. **MoSCoW** — Must / Should / Could / Won't (Won't required)
6. **성공 지표** — each KPI has a numeric target + measurement method
7. **스코프 경계 다이어그램** — Mermaid. In vs adjacent vs out
8. **오픈 질문** — blockers flagged for discussion

**Out of scope** (→ Design / ADR / tickets): implementation choices, tech stack, schedule.
...
6. **변경 영향 맵** — direct / indirect / no-ripple, by file or module
7. **AC 검증 전략** — for each `AC-NNN`, the verification method and threshold
8. **UI 명세** (frontend only) — screen list, state matrix (default/loading/empty/error), key interactions, a11y. Co-authored by Pixel Wizard, co-located here (not a separate UI Spec doc).
9. **위험 / 미해결** — known unknowns, fallbacks

**Out of scope**: business rationale (PRD), technology selection rationale (ADR), task breakdown (tickets).
...
3. **검토한 옵션** — minimum 3, including the chosen one. Each: pros, cons, cost.
4. **근거** — why the chosen option won
5. **결과 / 영향** — what this enables/forecloses, what teams/files must change
6. **원칙적 가이드라인** — derived rules for implementers (e.g., "always use parameterized queries through this layer")

A new ADR overriding an old one sets the old one's status to `superseded` and links forward.

### Diagnose — `docs/diagnose/<YYYY-MM-DD>-<topic-slug>.md`

**Author**: Galaxy Brain (or Technoking direct for quick triage). **Approval**: none — investigative output, not a decision.
...
| ADR | option-comparison diagram **only when** options interact spatially (architecture moves, layering shifts) — else a table is fine |
| Diagnose | timeline diagram **only when** ≥3 events interact across components |

No pro-forma diagrams. A diagram that doesn't earn its space goes.
...
Matches `orchestration-guide`'s Stop policy (B-pattern, shinpr-style); defer to it when in doubt.

## Reviewer expectations (Roastmaster's doc-quality bar)

BLOCKING if a doc:

- omits a required section above
- has ACs without measurable thresholds
- has an ADR with <3 options or no trade-offs
- mixes PRD and Design content in `large` (impl steps in PRD, business rationale in Design)
- has stale frontmatter (`updated` not bumped after substantive edits)
- ships commented-out or placeholder text as "draft"

Pass-with-comments is fine for diagrams that could be clearer, prose that could be tighter, or open questions with a reasonable owner. BLOCKING is reserved for missing structure or unverifiable claims.

## When this skill conflicts with the AC

If an AC explicitly waives a section ("no diagram needed for this trivial spec"), follow the AC and note the waiver in frontmatter under a `waivers:` list. Do not silently drop required structure.
