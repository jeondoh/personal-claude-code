...
Technoking's master playbook (others read for context; Technoking owns the lifecycle). `/feat` = 11 steps. `/task` = 3-step shortcut for verified-small work. Both share the same quality gates (Roastmaster review + codex adversarial review + auto-rescue, see `adversarial-review-bridge`).

## Pre-flight (every command)

Verify all three; any fail → halt with `run /setup-team first` or `run /codex:setup first`:

1. `.claude-team/config.yml` exists (`/setup-team` ran).
2. `/codex:status` healthy (hard dependency).
3. Panes alive per `registry.json`.
...
| **medium** | 3–5 files, small DB change OR 1–2 new API endpoints, within existing domain | `/feat` (1 Stop, merged spec) |
| **large** | 6+ files, both BE+FE, large DB change, new domain, OR external integration | `/feat` (3 Stops, separate docs) |

**Auto-large triggers** (any forces `large`, override file count): AuthN/AuthZ/permission change; DB schema migration (add table, drop column, change constraint); new domain (new bounded context); external payment provider / regulatory / compliance scope change.

**Auto-escalation mid-flight**: re-evaluate at every step. If new facts trip an auto-large trigger, escalate in-flight `medium` → `large`:
- Split merged PRD+Design into separate docs from this point (don't retrofit history).
- Insert missed Stops (e.g. catch up to step 3 PRD-approval Stop).
- Update umbrella ticket `complexity` field, notify user.
...
After step 6 batch approval (large) or step 5 merged approval (medium), step 7→11 is **autonomous** — no merge-time Stop. User can interrupt anytime; no interrupt = implicit consent.
...
`/task` skips PRD, Design, ADR, decomposition, parallel work, integration testing. It does **not** skip Roastmaster review, codex adversarial review, or auto-rescue.

## Quality gates — every ticket, every complexity

Invariant across `/feat` and `/task`, small/medium/large:

1. `/codex:adversarial-review --background` per PR (codex is sole reviewer).
2. Roastmaster (opus) judges codex result — uphold / downgrade BLOCKING → COMMENT / escalate. Never walks the diff itself.
...
Roastmaster authority is final; a BLOCKING halts merge until resolved or escalated.

## Worker escalation invariants

Workers never get stuck in silent retry loops. **Four** triggers fire `kind: error_2x` immediately (no debug retry), plus a Technoking-side safety net. Persona bodies (`persistence-paladin`, `pixel-wizard`, `what-if-witch`) evaluate the same four; values here are canonical.

| # | Trigger | Threshold | Detected by |
|---|---|---|---|
| 1 | `error_2x` — same `error_signature` repeats | After every build/test; match `last_error_signature` (formula in `adversarial-review-bridge § Error signature`) | Worker, before next code edit |
...
| 4 | `context_pressure` — worker's own context | **> 80%** while failure unresolved | Worker (pre-empt before judgment degrades) |

Retry unit = **one build/test command execution**, not a logical "cycle." Any trigger → worker writes inbox `kind: error_2x` with `reason: build_loop|timeout|attempt_limit|context_pressure` and halts.

**Safety net — watchdog daemon (v2)**: `technoking-watchdog-daemon.sh` (launched by `/setup-team`) calls `ticket-watchdog.sh <pane> --dispatch-surrogate` against every worker pane every 40s. The probe collects 6 signals (`error_loop`, `rev_repeat`, `rev_idle`, `last_update_stale`, `protected_breach`, `worktree_stagnation`), verifies, and classifies as `protected_breach` / `confirmed_loop` / `stagnation` / `ambiguous`. Confirmed verdicts auto-publish the kind-specific surrogate INBOX (`escalation_needed` / `error_2x` / `escalation_needed`) **and** touch `.runtime/<pane>.complete`; `ambiguous` publishes `pattern_question` **without** sentinel — worker stays alive and Technoking surfaces evidence via `AskUserQuestion`. **Technoking never calls the watchdog manually** — see `agents/technoking.md § Watchdog detection`. Triggers 1–4 are the worker's job; the watchdog is the safety net when they misfire.
...
Ticket batch approved as a set, not ticket-by-ticket. Format includes full ticket list with `id`, `title`, `assignee`, `complexity`, `depends_on`. After approval, Technoking runs through step 11 without further gates (modulo escalations).

### Forced escalations (Stop regardless of policy)

- Worker `kind: escalation_needed` with reason `requirements_change | architectural_change | untestable_ac`.
- Mid-flight auto-escalation to `large` (see above).
- 3rd consecutive review BLOCKING in step 9 (max rounds reached).
- Rescue validation fails (`reason: rescue_failed`).
- Codex unavailable (`reason: codex_unavailable`) — pre-flight catches this; if mid-flight:
  - **Halt** all new codex dispatches (adversarial-review + rescue). No degraded-mode review.
  - In-flight implementation workers **continue** (no direct codex dependency).
  - Notify user: "codex 다운 — PR 리뷰/rescue 대기 중. `/codex:setup` 재실행 후 알려주세요."
...
**Step 1 — Intake**: classify per rule table. Create umbrella ticket (`type: work`) in `tickets/queue/`, assignee `unassigned`. Move to `in-progress/` on start.

**Step 2 — PRD**: dispatch Spec Shaman as subagent (Agent tool, inline in `main`). PRD per `documentation-criteria`. `medium` → merged spec section, not own file.

**Step 4 — Design**: dispatch Galaxy Brain as subagent (Agent tool, inline in `main`). Reads `documentation-criteria` + `coding-principles`. ADRs per trigger rules.

**Step 6 — Decomposition**: non-overlapping `files_in_scope`; overlap → sequence via `depends_on`. Ticket ≤ ~400 LOC diff. `medium`: 1–2 tickets inline (no Stop). `large`: batch with approval Stop.

**Step 7 — Fail-first AC tests**: What-If Witch writes one acceptance test per AC, committed red before step 8. Untestable AC → `escalation_needed: untestable_ac`.
...
**Step 10 — Integration + E2E**: `large` default; `medium` per AC. What-If Witch runs integration suite. Max 2 retries → escalate.

**Step 11 — Merge**: pre-merge checklist (`git-flow`). Squash to `main`, move ticket → `done/`, remove worktree, post completion summary.
